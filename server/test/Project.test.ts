...
`
  ...
  const { entities } = project;
  expect(entities.getEntitiesInDocument("test").length).toBe(2);
  expect(entities.getEntitiesInDocument("test2").length).toBe(1);
  const decl = entities.getDeclaration("some-ref");
  expect(decl.name).toBe("some-ref");
  const references = entities.getReferences("some-ref");
  ...
  expect(project.documentCount).toBe(1);
  expect(entities.getEntitiesInDocument("test")).toBeUndefined();
  expect(entities.getDeclaration("some-ref")).toBeUndefined();
